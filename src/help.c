...
const char *kg_help_lines[] = {
	...
	"│ C-S-arrow  word/para    │ C-x r d  delete rect    │ C-s/C-r  search fwd/bk  │",
	"│ S-PgUp/Dn  extend page  │ C-x r c  clear rect     │ M-%      query replace  │",
	"│ C-w/S-Del  cut          │                         │ C-l      recenter       │",
	"│ M-w/C-Ins  copy         │ MACROS                  │ C-g      cancel         │",
	"│ C-y/S-Ins  paste        │ C-x (/F3 begin macro    │ C-h      help           │",
	"│ DEL        del region   │ C-x )/F4 end macro      │ C-z      suspend        │",
	"│ M-!/M-|    shell cmd ±  │ C-x e/F4 exec macro     │ C-u      numeric arg    │",
	"└─────────────────────────┴─────────────────────────┴─────────────────────────┘",
